...
import cn.com.antika.Jpush.RandomUUID;
import cn.com.antika.Thread.UpdateLoginInfoForAccountThread;
import cn.com.antika.adapter.ServicingInfoListAdapter;
...
public class HomePageActivity extends BaseActivity implements OnClickListener,OnItemClickListener,OnItemLongClickListener{
	...
	private Thread              requestWebSeriviceThread;
	...
	private NewRefreshListView.OnRefreshListener refreshListViewWithWebService;
	...
	private static class HomePageActivityHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			//包进行下载安装升级
				...
				homePageActivity.sortByTgStatusServicing.setTextColor(homePageActivity.getResources().getColor(R.color.title_font));
			...
			if (homePageActivity.requestWebSeriviceThread != null) {
				homePageActivity.requestWebSeriviceThread.interrupt();
				homePageActivity.requestWebSeriviceThread = null;
			}
		}
	}
	...
	private void initView() {
		...
		refreshListViewWithWebService = new NewRefreshListView.OnRefreshListener() {
					...
					if (!qrCodeArray[0].equals(userInfoApplication.getAccountInfo().getCompanyCode()))
						...
						if (qrCodeArray[1].equals(CUSTOMER_CODE)) {
							...
							if (authMyCutomserInfoRead == 0) {
								...
								getCustomerInfoByWebService(requestWebSeriviceThread, mHandler,customerQRCodeArray[1],String.valueOf(userInfoApplication.getAccountInfo().getAccountId()));
							}
							...
							if (authServiceRead == 0) {
								...
								getProductInfoByWebService(requestWebSeriviceThread, mHandler,customerQRCodeArray[1],
							...
							if (authCommodityRead == 0) {
								...
								getProductInfoByWebService(requestWebSeriviceThread,mHandler,customerQRCodeArray[1],String.valueOf(userInfoApplication.getAccountInfo().getAccountId()),1);
							}
						}
	...
	private void updateLoginInfoByWebService(){
		...
		requestWebSeriviceThread = new UpdateLoginInfoForAccountThread(mHandler, uuidRaw,userInfoApplication);
		requestWebSeriviceThread.start();
	}
	private void getServicingList(){
		...
		requestWebSeriviceThread = new Thread() {
			@Override
			public void run() {
				// TODO Auto-generated method stub
	...
	public void onClick(View v) {
		...
		case R.id.sort_by_tg_status_servicing:
			...
			sortByTgStatusServicing.setTextColor(getResources().getColor(R.color.title_font));
		...
		case R.id.sort_by_tg_status_completed:
			...
			sortByTgStatusCompleted.setTextColor(getResources().getColor(R.color.title_font));
			for(OrderInfo oi:servicingInfoList){
					if(servicePICID==0){
		...
		case R.id.sort_by_tg_status_all:
			...
			sortByTgStatusAll.setTextColor(getResources().getColor(R.color.title_font));
			sortByTgStatusServicing.setTextColor(getResources().getColor(R.color.white));
			sortByTgStatusCompleted.setTextColor(getResources().getColor(R.color.white));
