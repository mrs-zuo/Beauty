...
import cn.com.antika.manager.NoteModel;
import cn.com.antika.manager.RecordModel;
...
public class OrderAdvancedSearchActivity extends BaseActivity implements OnClickListener {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.advanced_search_make_sure_btn:
                ...
                AdvancedSearchCondition.OrderAdvancedSearchConditionBuilder advancedSearchCondition = new AdvancedSearchCondition.OrderAdvancedSearchConditionBuilder();
                //按时间筛选,时间必须满足的条件
    ...
    private void saveConditionToSharePre(AdvancedSearchCondition currentCondition) {
        ...
        String key = "";
        if (conditionFlag == RECORD_CONDITION) {
            key = sharePreUtil.getAdvancedConditionKey(mBranchID, accountID, SharedPreferenceUtil.AdvancedFilterFlag.RecordFLag);
        } else if (conditionFlag == NOTE_CONDITION) {
            key = sharePreUtil.getAdvancedConditionKey(mBranchID, accountID, SharedPreferenceUtil.AdvancedFilterFlag.NoteFlag);
        }
