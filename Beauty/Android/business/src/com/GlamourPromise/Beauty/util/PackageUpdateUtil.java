...
import com.GlamourPromise.Beauty.Business.SplashActivity;
...
public class PackageUpdateUtil {
	...
	public void getApkDownLoadURL() {
		...
		try {
			getAndroidURLJson.put("ClientID","GP");
		} catch (JSONException e) {
			e.printStackTrace();
		}
		Thread requestWebServiceThread = new Thread() {
		...
		Thread requestWebServiceThread = new Thread() {
			...
			public void run() {
				try {
					...
					File file = fileCache.getFileWithType("com.glamourpromise.beauty.business", ".apk");
	...
	public void installNewApk() {
		...
		String filename = "com.glamourpromise.beauty.business";
		...
		if (Build.VERSION.SDK_INT >= 24) {
			Uri apkUri = FileProvider.getUriForFile(mContext, "com.GlamourPromise.Beauty.Business.fileprovider", outputFile);
