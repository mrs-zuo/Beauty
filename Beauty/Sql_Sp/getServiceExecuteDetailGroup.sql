...
if object_id('getServiceExecuteDetailGroup', 'p') is not null
begin
  drop procedure getServiceExecuteDetailGroup
end
go

create procedure getServiceExecuteDetailGroup
  @CompanyID integer,       --门店ID
     ...
     FROM (
       select
           ...
           CASE ISNULL(T11.IsUseRate ,2)
             WHEN 1 THEN
               CASE T11.Type
                 WHEN 1 THEN T12.PaymentAmount * T12.ProfitRate
               ELSE T12.PaymentAmount * T12.ProfitRate * -1 END
       ...
       AND T1.ExecutorID in (
          ...
          from
              [ACCOUNT] P1
              INNER JOIN [TBL_USER_TAGS] P2 ON P1.UserID = P2.UserID AND P2.Available = 1
          WHERE
       ...
       GROUP BY T3.Code,T1.GroupNo,T3.TGTotalCount,T3.SumSalePrice,T11.Type,T11.IsUseRate,T3.OrderID,T12.PaymentAmount,T12.ProfitRate,T12.ID,T1.GroupNo
