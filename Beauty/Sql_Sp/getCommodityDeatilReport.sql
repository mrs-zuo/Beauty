...
begin
   ...
   select T22.PaymentID,
          ...
          (T22.TotalSalePrice*(
            T23.PayCashRate + T23.PayMoneyBalanceRate + T23.PayBankCardRate + T23.PayOtherRate  + T23.PayWeChatBalanceRate + T23.PayAlipayBalanceRate + T23.PayLoanRate + T23.PayThirdRate)+
            T23.PayCash + T23.PayMoneyBalance + T23.PayBankCard + T23.PayOther  + T23.PayWeChatBalance + T23.PayAlipayBalance + T23.PayLoan + T23.PayThird
          ) ProfitAmount
   from
   (SELECT
        T5.PaymentID,
    ...
    FROM ( 
        SELECT 
            ...
            CASE 
                when T11.DeliveredAmount = 0  then '未交付' 
                when T11.DeliveredAmount = T11.Quantity then '已交付' 
                else '部分交付' 
            end isSend ,
            T3.Remark ,
            CASE 
        ...
        FROM [TBL_ORDERPAYMENT_RELATIONSHIP] T1 WITH (NOLOCK)  
            INNER JOIN [ORDER] T2 ON T2.ID = T1.OrderID AND T2.PaymentStatus > 0 
            INNER JOIN [TBL_ORDER_COMMODITY] T11 ON T2.ID = T11.OrderID 
            ...
            INNER JOIN (
                SELECT Count(T9.SlaveID) PeopleCount,
                T9.MasterID  
                FROM [TBL_PROFIT] T9 
                WHERE T9.Available = 1 AND T9.Type = 2 
                Group by T9.MasterID
        ...
        where
            T11.CompanyID =@CompanyID 
        AND T3.PaymentTime >=@BeginDay 
        AND T3.PaymentTime <= @EndDay 
        AND T3.PaymentTime >= T4.StartTime  
        AND  (
             (T3.Type = 1 AND (T3.Status = 2 OR T3.Status = 3)) 
             OR (T3.Type = 2 AND (T3.Status = 6 OR T3.Status = 7))
             )  
        ...
        left join [COMMODITY] T8 WITH (NOLOCK) ON T5.ProductID = T8.ID 
    ...
    (select T21.PaymentID,
     ...
     from (select distinct T20.PaymentID,
           ...
           from (Select distinct T1.PaymentID,
                 ...
                 INNER JOIN [ORDER] T4 ON T3.OrderID = T4.ID  AND T4.ProductType = 1 AND T4.PaymentStatus > 0 
