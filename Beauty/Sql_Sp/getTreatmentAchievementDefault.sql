...
if object_id('getTreatmentAchievementDefault', 'p') is not null
begin
  drop procedure getTreatmentAchievementDefault
end
go

create procedure getTreatmentAchievementDefault
     ...
     FROM (
         select
           ...
           CASE ISNULL(T6.IsUseRate ,2)
             WHEN 1 THEN
               --支付金额 * 业绩折算率
               SUM(CASE T6.Type WHEN 1 THEN T9.PaymentAmount * T9.ProfitRate ELSE T9.PaymentAmount * T9.ProfitRate * -1 END)
               --最终售价
           ELSE T8.SumSalePrice END Price,
           T8.OrderID,
           T8.TGTotalCount
         from
         ...
         GROUP BY T8.OrderID,T8.TGTotalCount,T8.SumSalePrice,T6.IsUseRate) T11
