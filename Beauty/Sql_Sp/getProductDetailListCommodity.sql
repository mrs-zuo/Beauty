...
if object_id('getProductDetailListCommodity', 'p') is not null
begin
  drop procedure getProductDetailListCommodity
end
go

create procedure getProductDetailListCommodity
...
--服务销售分析-商品Commodity productType != 0 obejectType = 0（个人/员工）
SELECT
    T8.ObjectName,
...
(
  ...
  from (
    select
		 sum(T1.PaymentAmount * T1.ProfitRate * T1.typeflag * dbo.getProfitrate(T4.SlaveID, T4.MasterID,1)) as TotalPrice
        ,T2.Code Code
