...
NSString *const TaskList_ViewControllerFilterData = @"TaskList_ViewControllerFilterData";

@interface TaskList_ViewController ()<UITableViewDataSource,UITableViewDelegate,ZXAppointmentFilterViewControllerDelegate>
...
@implementation TaskList_ViewController
...
{
    NSData *date =[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"TaskFilterDoc_ACCOUNT-%ld-BRANCH-%ld-%@",(long)ACC_ACCOUNTID,(long)ACC_BRANCHID,TaskList_ViewControllerFilterData]];
    ...
    ZXAppointmentFilterViewController *filter = [[ZXAppointmentFilterViewController alloc] init];
...
//    cell.statusLable.text = [NSString stringWithFormat:@"%@|%@",appointDoc.Appointment_customer,appointDoc.Appointment_TaskTypeStr];
    ...
    cell.statusLable.text =  appointDoc.Appointment_customer;
...
- (void)dismissViewControllerWithDoc:(TaskFilterDoc *)taskFilter
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:data forKey:[NSString stringWithFormat:@"TaskFilterDoc_ACCOUNT-%ld-BRANCH-%ld-%@",(long)ACC_ACCOUNTID,(long)ACC_BRANCHID,TaskList_ViewControllerFilterData]];
...
-(void)requestList
{
    ...
    NSMutableArray * arr = [[NSMutableArray alloc] initWithObjects:@"2",nil];
//    NSDictionary * par = @{
...
//                           };
    if (filterDoc.TaskTypeArrs) {
        ...
        [filterDoc.TaskTypeArrs addObject:@(1)]; // 服务预约
    }
    NSDictionary * par = @{
                           @"BranchID":@((long)branchId),
