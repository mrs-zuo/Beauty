...
@interface PCategoryTwoViewController ()
...
@implementation PCategoryTwoViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        if (categoryDoc.NextCategoryCount == 0) {
            ...
            PCategoryOneViewController *categoryOneVC = [self.storyboard instantiateViewControllerWithIdentifier:@"PCategoryOneViewController"];
...
- (void)requestGetCategoryListByParentId:(NSInteger)parentId
{
    ...
    _requestCategoryListByCompanyIDOperation = [[GPBHTTPClient sharedClient] requestUrlPath:@"/Category/getCategoryListByCategoryID" andParameters:par failureHandle:AFRequestFailureDefault WithSuccess:^(id json) {
        [SVProgressHUD dismiss];
        if (!categoryArray) {
            categoryArray = [NSMutableArray array];
        } else {
