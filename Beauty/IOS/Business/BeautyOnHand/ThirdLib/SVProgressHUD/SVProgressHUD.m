...
//+ (void)showSuccessWithStatus2:(NSString *)string
//{
//    [[self sharedView] showProgress:-10 status:string maskType:SVProgressHUDMaskTypeBlack];
//}
//
//+ (void)showErrorWithStatus2:(NSString *)string
//{
//     [[self sharedView] showProgress:-11 status:string maskType:SVProgressHUDMaskTypeBlack];
//}
//GPB-1434 追加 设置时间到或点击跳转执行
+ (void)showSuccessWithStatus2:(NSString *)string duration:(NSTimeInterval)duration touchEventHandle:(void(^)())handle
{
    touchEventHandle = handle;
    [[self sharedView] showProgress:-10 status:string duration:duration maskType:SVProgressHUDMaskTypeBlack];
}

+ (void)showSuccessWithStatus2:(NSString *)string touchEventHandle:(void(^)())handle
{
    touchEventHandle = handle;
    [[self sharedView] showProgress:-10 status:string maskType:SVProgressHUDMaskTypeBlack];
}

+ (void)showErrorWithStatus2:(NSString *)string touchEventHandle:(void(^)())handle
{
    touchEventHandle = handle;
    [[self sharedView] showProgress:-11 status:string maskType:SVProgressHUDMaskTypeBlack];
}

+ (void)showErrorWithStatus2:(NSString *)string duration:(NSTimeInterval)duration touchEventHandle:(void(^)())handle
{
    touchEventHandle = handle;
    [[self sharedView] showProgress:-11 status:string duration:duration maskType:SVProgressHUDMaskTypeBlack];
}
...
- (void)overlayViewDidReceiveTouchEvent {
    ...
    //test
    NSLog(@"the fadeOutTimer is %d----%@", self.fadeOutTimer.isValid, self.fadeOutTimer);
    if (self.fadeOutTimer.isValid) {
        [self.fadeOutTimer invalidate];
    }
    NSLog(@"the fadeOutTimer is %d----%@", self.fadeOutTimer.isValid, self.fadeOutTimer);

    if (self.progress == -10 || self.progress == -11) {
        touchEventHandle();
    }
}

#pragma mark - Master show/dismiss methods
- (void)showProgress:(float)progress status:(NSString*)string duration:(NSTimeInterval)duration maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(progress >= 0) {
        ...
        self.imageView.image = [self hudSuccessImage];
        self.imageView.hidden = NO;
        [self.spinnerView stopAnimating];
        [self cancelRingLayerAnimation];
    } else if (progress == -11) {
        self.imageView.image = [self hudErrorImage];
        self.imageView.hidden = NO;
        [self.spinnerView stopAnimating];
        [self cancelRingLayerAnimation];
    }
    ...
    self.fadeOutTimer = [NSTimer timerWithTimeInterval:duration target:self selector:@selector(goToBlock) userInfo:nil repeats:NO];
