...
@property (weak, nonatomic) PictureDisplayView *otherPictureDisplayView;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        picScrollView = [[GPScrollView alloc] initWithFrame:self.bounds];
...
{
    float content_W = picScrollView.contentSize.width;
    float rect_W = picScrollView.frame.size.width;
    float currentSize_W = picScrollView.contentOffset.x;
...
{
    float rect_W = picScrollView.frame.size.width;
    float currentOffset_Y = picScrollView.contentOffset.x;
...
{
    picScrollView.bindPictureDisplayView = otherPictureDisplayView;
    _otherPictureDisplayView = otherPictureDisplayView;
}

- (NSArray *)getDeleteImageURLsArray
{
    NSArray *array = self.deleteImgs;
    self.deleteImgs = nil;
    return array;
}
...
- (void)addImageViewByURLStr:(NSString *)imgURL animated:(BOOL)animated
{
    ...
    float imgView_X = (picSize.width + spacing) * imgCount;
    float imgView_Y = margin;
    ...
    float content_W = self.picScrollView.contentSize.width;
    float content_H = self.picScrollView.contentSize.height;
    if (imgCount == 0) {
...
{
    float deleteImg_X = theImageView.frame.size.width - 20.0f;
    float deleteImg_Y = 0.0f;
...
{
    if (imgsURLs.count >= 8) {
        [SVProgressHUD showErrorWithStatus:@"一次最多只能上传8张照片"];
        return;
    }
...
{
    UIImageView *imgeView = (UIImageView *)gestureRecognizer.view;
    if ([self.delegate respondsToSelector:@selector(pictureDisplayView:selectedView:)]) {
        [self.delegate pictureDisplayView:self selectedView:imgeView];
    }
...
{
    float contentOffSet_X = self.picScrollView.contentOffset.x;
    float contentSize_W = self.picScrollView.contentSize.width;
    float rect_W = self.picScrollView.frame.size.width;
...
-(void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView
{
    [scrollView setContentOffset:scrollView.contentOffset animated:YES];
     [_otherPictureDisplayView.picScrollView setContentOffset:scrollView.contentOffset animated:YES];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            ...
            [imgEditViewController presentViewController:cameraController animated:YES completion:^{}];
        }
        ...
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
            ...
            [[UINavigationBar appearance] setBackgroundImage:nil forBarMetrics:UIBarMetricsDefault];
            [imgEditViewController presentViewController:photoController animated:YES completion:^{}];
        }
    }
}
