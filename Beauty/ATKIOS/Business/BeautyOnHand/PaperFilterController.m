...
@interface PaperFilterController ()<UITableViewDelegate, UITableViewDataSource, SelectCustomersViewControllerDelegate>
...
@implementation PaperFilterController
...
- (NSString *)path  {
    ...
    return  [documentsDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@_%ld_%ld", @"Paper", (long)ACC_ACCOUNTID, (long)ACC_BRANCHID]];
}
...
- (void)initView
{
    ...
    NavigationView *navigationView = [[NavigationView alloc] initWithPosition:CGPointMake(0.0f, kORIGIN_Y + 5.0f) title:@"专业筛选"];
...
- (void)personAndCustomerSelector
{
    ...
    [personView setSelectModel:(self.selectType == SELECTIONCUSTOMER ? 0 : 1) userType:self.selectType customerRange:(self.selectType == SELECTIONCUSTOMER ? CUSTOMEROFMINE :(kMenu_Type == 1 ? CUSTOMEROFMINE : CUSTOMERINBRANCH)) defaultSelectedUsers:(self.selectType == SELECTIONCUSTOMER ? @[user] : self.priFilter.personsArray)];
