...
@implementation GDataXMLDocument (ParseXML)

//+ (void)parseXML:(NSString *)xml viewController:(UIViewController *)viewController showSuccessMsg:(BOOL)showSuccessMsg showFailureMsg:(BOOL)showFailureMsg
...
//    
//    NSInteger flag = [[resultArray objectAtIndex:0] intValue];
//    NSString *msg = [resultArray objectAtIndex:1];
//    if (flag > 0) {
//        if (showSuccessMsg && [msg length] > 0) {
//            [SVProgressHUD showSuccessWithStatus2:msg];
//        }
//        success(flag);
//        
...
//    } else {
//        if (showFailureMsg) {
//            [SVProgressHUD showErrorWithStatus2:msg];
//        }
//        failure();
//    }
//}

+ (void)parseXML2:(NSString *)xml success:(void (^)(GDataXMLElement *, NSString *))success failure:(void (^)(NSString *mesg, NSInteger errorFlag))failure
{
    __autoreleasing GDataXMLDocument *document = [[GDataXMLDocument alloc] initWithXMLString:xml encoding:NSUTF8StringEncoding error:0];
    ...
    if (result_Flag == 1) {
        GDataXMLElement *contentElement = [[[resultData elementsForName:@"Content"] objectAtIndex:0] copy];
        success(contentElement, result_Msg);
    } else {
        if (result_Msg.length) {
            [SVProgressHUD showErrorWithStatus2:result_Msg touchEventHandle:^{}];
        } else {
...
+ (void)parseXML2:(NSString *)xml viewController:(UIViewController *)viewController showSuccessMsg:(BOOL)showSuccessMsg showFailureMsg:(BOOL)showFailureMsg success:(void (^)(GDataXMLElement *, NSString *))success failure:(void (^)())failure;
{
    __autoreleasing GDataXMLDocument *document = [[GDataXMLDocument alloc] initWithXMLString:xml encoding:NSUTF8StringEncoding error:0];
    __autoreleasing GDataXMLElement *resultData = [[document nodesForXPath:@"//Result" error:nil] objectAtIndex:0];
    NSInteger result_Flag   = [[[[resultData elementsForName:@"Flag"] objectAtIndex:0] stringValue] intValue];
    ...
    if (result_Flag == -1) {
        ...
        if (viewController) {
            if (showSuccessMsg && [result_Msg length] > 0 ) {  //直接显示操作成功信息 然后跳转到前一viewController （只适应与success没有跳转操作）
                [SVProgressHUD showSuccessWithStatus2:result_Msg duration:2.0 touchEventHandle:^{
