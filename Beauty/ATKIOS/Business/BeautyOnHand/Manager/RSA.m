...
//        sharedRSA = [[super allocWithZone:NULL] init];
//    });
//    return sharedRSA;
//}
            ...
            if (status == errSecSuccess)
            {
                ...
                if(status == errSecSuccess)
                {
                    ...
                    if(status == errSecSuccess && (trustResult == kSecTrustResultUnspecified || trustResult == kSecTrustResultProceed))
                    {
                        ...
                        if(_publicKey)
                            NSLog(@"Got public key successfully!");
                        if (cert)
                            CFRelease(cert);
                        if (policy)
                            CFRelease(policy);
...
- (NSData *)RSAEncryptWithData:(NSData *)plainData usingPublicKey:(BOOL)yes
{
    ...
    for(int i  = 0; i < blockCount ; i ++)
    {
        ...
        OSStatus status = SecKeyEncrypt(key, kSecPaddingPKCS1, [dataSegment bytes], dataSegmentRealSize, cipherBuffer, &cipherBufferSize);
...
-(NSData *)RSADEcryptWithData:(NSData *)cipherData usingPrivateKey:(BOOL)yes
{
    ...
    for (int i = 0; i < blockCount; i++) {
        ...
        OSStatus status  = SecKeyDecrypt(key, kSecPaddingPKCS1, [dataSegment bytes], dataSegmentSize, plainBuffer, &plainBufferSize);
        if (status == errSecSuccess)
        {
