...
- (void)setTitles:(NSArray *)titles andObjects:(NSMutableArray *)objects withTags:(NSArray *)tags{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    SurplusTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    
    if (!cell) {
        cell = [[SurplusTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
...
- (CGFloat)heightForRowAtIndex:(long)index{
    if([_delegate respondsToSelector:@selector(heightForRowAtIndex:)]){
        CGFloat f = [_delegate heightForRowAtIndex:index];
        return f;
    }else{
        return 44;
    }
