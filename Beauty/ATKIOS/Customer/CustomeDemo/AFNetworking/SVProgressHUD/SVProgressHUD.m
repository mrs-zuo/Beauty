...
+ (void)showSuccessWithStatusWithToch:(NSString *)string touchEventHandle:(void(^)())handle
{
...
- (void)showProgress:(CGFloat)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    self.fadeOutTimer = [NSTimer timerWithTimeInterval:3 target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
...
- (NSTimeInterval)displayDurationForString:(NSString*)string
{
    return (CGFloat)string.length*0.06 + 0.3;
}
